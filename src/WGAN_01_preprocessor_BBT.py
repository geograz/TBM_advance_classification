...
Preprocessing functions for the Brenner Base Tunnel (Exploratory Tunnel)
TBM dataset
"""
...
def concat():
    ...
    folder = (r'00_data\01_raw\01_BBT')
    ...
    df, files = utils.concat_tables_BBT(folder, drop_standstills=drop_standstills,
                             check_for_miss_vals=check_for_miss_vals)

    if drop_standstills is False:
        df.to_parquet(r'00_data\02_combined\BBT_TBMdata_wStandstills.gzip', index=False)
    else:
        df.to_parquet(r'00_data\02_combined\BBT_TBMdata.gzip', index=False)
...
# import pandas as pd
# df = pd.read_parquet(r'00_data\02_combined\BBT_TBMdata.gzip')
...
def preprocessor(df):
    ...
    np.save(fr'00_data\03_train\BBT\BBT_train_X_pene_adv_force_torque_ucs_{look_back}.npy', train_data)
    dump(scaler_pene, open(fr'00_data\03_train\BBT\BBT_scaler_pene_{look_back}.pkl', 'wb'))
    dump(scaler_adv_force, open(fr'00_data\03_train\BBT\BBT_scaler_adv_force_{look_back}.pkl', 'wb'))
    dump(scaler_torque, open(fr'00_data\03_train\BBT\BBT_scaler_torque_{look_back}.pkl', 'wb'))
    dump(scaler_ucs, open(fr'00_data\03_train\BBT\BBT_scaler_ucs_{look_back}.pkl', 'wb'))
