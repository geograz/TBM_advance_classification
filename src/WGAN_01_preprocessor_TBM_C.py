...
Preprocessing functions for the TBM_C dataset
...
def concat():
    ...
    folder = (r'00_data\01_raw\02_TBM_C')
    ...
    df, files = utils.concat_tables_TBM_C(folder, drop_standstills=drop_standstills,
                             check_for_miss_vals=check_for_miss_vals)

    if drop_standstills is False:
        df.to_parquet(r'00_data\02_combined\TBM_C_TBMdata_wStandstills.gzip', index=False)
    else:
        df.to_parquet(r'00_data\02_combined\TBM_C_TBMdata.gzip', index=False)
...
# import pandas as pd
# df = pd.read_parquet(r'00_data\02_combined\TBM_C_TBMdata.gzip')
...
def preprocessor(df):
    ...
    np.save(fr'00_data\03_train\TBM_C\TBM_C_train_X_pene_adv_force_torque_ucs_{look_back}.npy', train_data)
    dump(scaler_pene, open(fr'00_data\03_train\TBM_C\TBM_C_scaler_pene_{look_back}.pkl', 'wb'))
    dump(scaler_adv_force, open(fr'00_data\03_train\TBM_C\TBM_C_scaler_adv_force_{look_back}.pkl', 'wb'))
    dump(scaler_torque, open(fr'00_data\03_train\TBM_C\TBM_C_scaler_torque_{look_back}.pkl', 'wb'))
    dump(scaler_ucs, open(fr'00_data\03_train\TBM_C\TBM_C_scaler_ucs_{look_back}.pkl', 'wb'))
    
    return df, scaler_pene, scaler_adv_force, scaler_torque, scaler_ucs


df_TBM_C, TBM_C_scaler_pene, TBM_C_scaler_adv_force, TBM_C_scaler_torque, TBM_C_scaler_ucs = preprocessor(df)
